...
(async () => {
  const symbol = 'CHIPUSDT';
  const t = (await pub('/v5/market/tickers', { category: 'linear', symbol })).result?.list?.[0];
  console.log('=== CHIP Ticker ===');
  console.log(`Price: ${t.lastPrice}  24h: ${(parseFloat(t.price24hPcnt)*100).toFixed(2)}%  H: ${t.highPrice24h}  L: ${t.lowPrice24h}`);
  console.log(`Vol: $${(parseFloat(t.turnover24h)/1e6).toFixed(2)}M  OI: $${(parseFloat(t.openInterestValue)/1e6).toFixed(2)}M  Funding: ${(parseFloat(t.fundingRate)*100).toFixed(4)}% (${t.fundingIntervalHour}h)`);
  ...
  (kd.result?.list || []).reverse().forEach(r => {
    ...
    console.log(`${dt}  O:${r[1]}  H:${r[2]}  L:${r[3]}  C:${r[4]}  ${pct}%  V:${(parseFloat(r[5])/1000).toFixed(0)}K`);
  });
