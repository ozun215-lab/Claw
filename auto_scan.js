...
async function updateDashboard(longCandidates, shortCandidates) {
  try {
    const data = {
      ...
      alerts: [{
        timestamp: new Date(),
        message: `✅ 자동 스캔: 롱 ${longCandidates.length}개, 숏 ${shortCandidates.length}개`
      }]
    };
    ...
    if (res.ok) {
      console.log(`[${new Date().toLocaleTimeString()}] ✅ 대시보드 업데이트: 롱 ${longCandidates.length}개, 숏 ${shortCandidates.length}개`);
    }
  } catch (e) {
    console.log(`[${new Date().toLocaleTimeString()}] ⚠️  대시보드 연결 실패`);
  }
}
...
async function runScan() {
  ...
  console.log(`[${new Date().toLocaleTimeString()}] ✅ 스캔 완료`);
  if (longs.length > 0) console.log(`   🟢 롱 ${longs.length}개: TOP ${longs[0].symbol} (${longs[0].score}점)`);
  if (shorts.length > 0) console.log(`   🔴 숏 ${shorts.length}개: TOP ${shorts[0].symbol} (${shorts[0].score}점)`);
...
console.log(`
╔════════════════════════════════════════════════════════════╗
║        🤖 Auto Scan Scheduler - 30분 간격 자동 스캔        ║
╚════════════════════════════════════════════════════════════╝

⏰ 스캔 일정:
   - 매 30분마다 자동 실행
   - 다음 첫 스캔: ${new Date(Date.now() + 30 * 60000).toLocaleTimeString()}

📊 대시보드: http://localhost:3000 (폰: http://<IP>:3000)
...
// 프로세스 종료 처리
process.on('SIGINT', () => {
  console.log(`\n[${new Date().toLocaleTimeString()}] 🛑 스캐줄러 중지됨`);
  process.exit(0);
});
