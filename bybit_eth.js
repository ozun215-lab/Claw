...
(async () => {
  ...
  console.log(`Price: ${t.lastPrice}  24h: ${(parseFloat(t.price24hPcnt)*100).toFixed(2)}%  H: ${t.highPrice24h}  L: ${t.lowPrice24h}`);
  console.log(`OI: $${(parseFloat(t.openInterestValue)/1e6).toFixed(0)}M  Vol: $${(parseFloat(t.turnover24h)/1e6).toFixed(0)}M  Funding: ${(parseFloat(t.fundingRate)*100).toFixed(4)}% (${t.fundingIntervalHour}h)`);

  console.log('\n=== 1D Klines (last 20) ===');
  const kd = await pub('/v5/market/kline', { category: 'linear', symbol, interval: 'D', limit: '20' });
  ...
  console.log('\n=== 4h Klines (last 18) ===');
  const k4 = await pub('/v5/market/kline', { category: 'linear', symbol, interval: '240', limit: '18' });
  ...
  console.log('\n=== Funding (last 8) ===');
  const fr = await pub('/v5/market/funding/history', { category: 'linear', symbol, limit: '8' });
  ...
  console.log('\n=== OI (4h, last 8) ===');
  const oi = await pub('/v5/market/open-interest', { category: 'linear', symbol, intervalTime: '4h', limit: '8' });
  (oi.result?.list || []).reverse().forEach(o => {
    console.log(`${new Date(parseInt(o.timestamp)).toISOString().slice(0,16).replace('T',' ')}  OI=${(parseFloat(o.openInterest)/1e3).toFixed(0)}K`);
  });
