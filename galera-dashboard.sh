...
MYSQL_PASS="${MYSQL_PASS:-gooroom}"
REFRESH_INTERVAL=5 # 초 단위
SOCKET_PATH="/GPMS/DBMS/DATA/mysql.sock"
...
run_mysql_query() {
 ...
 mysql --socket="$SOCKET_PATH" -u "$MYSQL_USER" -p"$MYSQL_PASS" -sN -e "$query" 2>/dev/null || echo "ERROR"
}
...
main() {
 echo "Connecting to MySQL via socket: $SOCKET_PATH..."
 ...
 if [ "$data" = "ERROR" ]; then
 echo -e "${RED}❌ Cannot connect to MySQL via socket: $SOCKET_PATH${NC}"
 echo ""
